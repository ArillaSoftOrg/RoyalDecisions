...
namespace RoyalDecisions.Presentation
{
    ...
    /// scene keeps its own gold palette — <c>SceneSetupAutomation</c> decouples every place that
    /// used to read these values by reference (<c>ButtonColour</c>, the tap-choice-button fill) so
    /// none of this bleeds into gameplay. Mixes several wasteland tones rather than one colour
    /// repeated everywhere: rust for actions, ash for neutral surfaces, dried blood for danger —
    /// <c>SceneSetupAutomation.MenuToxicAccentColour</c> carries the one distinctly "zombie"
    /// toxic-green note, kept to slider fills only so it reads as a highlight, not the base.
    /// </remarks>
    public static class SettingsPanelTheme
    {
        ...
        /// ordinary settings around it — a dried-blood red rather than the rust used for every
        ...
        /// <summary>
        /// The ring colour used around the slider handle — dulled to an oxidised brass rather than
        /// bright gold, so it reads as worn/scavenged metal rather than an ornate-royal accent.
        /// </summary>
        public static readonly Color BorderGoldColour = new Color32(0x8A, 0x6A, 0x38, 0xFF);
    }
}
